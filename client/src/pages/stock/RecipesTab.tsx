...
/** Convert Excel serial date (e.g.46210) to DD.MM.YYYY string */
...
/** Normalize date: convert Excel serial or return as-is */
function normalizeDate(date: string): string {
  if (!date) return '—';
  // Check if it's an Excel serial date (pure number)
  if (/^\d{4,5}$/.test(date.trim())) {
    return excelSerialToDate(date.trim());
  }
  return date;
}
...
export default function RecipesTab({
  ...
  // Суммарный тоннаж выбранных рецептов
  const selectedTonnage = useMemo(() => {
    if (sel.size === 0) return 0;
    return sorted
      .filter(r => sel.has(r.recipe_uid))
      .reduce((sum, r) => sum + (r.batch_t || 0), 0);
  }, [sel, sorted]);
  ...
  const changeStatus = async (uid: string, action: Action) => {
    ...
    setBusy(true);
    try {
      await axios.post(`${API}/recipes/${encodeURIComponent(uid)}/status`, { status: action });
  ...
  const confirmArchive = async () => {
    ...
    setBusy(true);
    try {
      const res = await axios.post(`${API}/recipes/${encodeURIComponent(archiveUid)}/partial-archive`, { produced_tons: produced });
  ...
  const editTons = async (r: Recipe) => {
    ...
    setBusy(true);
    try {
      await axios.post(`${API}/recipes/${encodeURIComponent(r.recipe_uid)}/tons`, { tons });
  ...
  const deleteRecipe = async (uid: string) => {
    ...
    setBusy(true);
    try {
      await axios.delete(`${API}/recipes/${encodeURIComponent(uid)}`);
      flash('✅ Рецепт удалён');
      await reload();
    } catch (e: any) {
      flash(`❌ ${e.response?.data?.error || 'Ошибка'}`);
          ...
          <thead className="bg-gray-900 text-gray-400 text-xs">
            <tr>
              ...
              <th className="text-right px-3 py-2">
                Выработка, т
                {sel.size > 0 && (
                  <span className="ml-2 text-green-400 font-normal">Выбрано {fmt(selectedTonnage)} т</span>
                )}
              </th>
          ...
          <tbody>
            {listExpanded && sorted.map(r => (
              <tr key={r.recipe_uid} className="border-t border-gray-800">
                ...
                <td className="px-3 py-1.5 text-gray-400">{normalizeDate(r.date)}</td>
