...
export async function getRecipesList() {
  const result = await db.execute(sql`SELECT id, recipe_uid, code, name, status, batch_t, base_batch_kg, active_from, is_partial FROM recipe`);
...
function stockSignal(plant_qty: number, lip_qty: number, consumed: number): "critical" | "transfer" | "ok" {
  const available = round3(plant_qty + lip_qty - consumed);
  if (available < -1e-6) return "critical";
  if (round3(plant_qty - consumed) < -1e-6) return "transfer";
  return "ok";
}
 ...
 *   архив     → archived   (does NOT consume, stock already deducted)
 *   отменён   → cancelled  (does NOT consume, stock RETURNED)
...
export async function pgWriteRecipe(recipe: {
  ...
  return await db.execute(sql`BEGIN`).then(async () => {
    try {
      // 1. Create recipe
      // Get next version for this code
      const maxVerResult = await db.execute(sql`SELECT COALESCE(MAX(version), 0) + 1 as next_version FROM recipe WHERE code = ${recipe.code}`);
      const nextVersion = (maxVerResult.rows[0] as any).next_version;

      const recResult = await db.execute(sql`
        INSERT INTO recipe (recipe_uid, code, name, status, version, batch_t, base_batch_kg, active_from)
        VALUES (${recipe.code + '_' + Date.now()}, ${recipe.code}, ${recipe.full_name}, 'active', ${nextVersion}, ${recipe.batch_t}, ${recipe.base_batch_kg}, ${recipe.date})
      ...
      for (const line of recipe.lines) {
        ...
        await db.execute(sql`
          ...
          VALUES (${recipeId}, ${skuId}, ${period}, ${line.consumption_kg}, 0, ${line.consumption_kg}, ${nextVersion}, ${new Date().toISOString()})
        `);
      }
...
export async function pgRewriteRecipeItems(recipeUid: string, lines: {
  ...
  const recResult = await db.execute(sql`SELECT id, version FROM recipe WHERE recipe_uid = ${recipeUid}`);
  if (recResult.rows.length === 0) return;
  const recipeId = (recResult.rows[0] as any).id;
  const recipeVersion = (recResult.rows[0] as any).version || 1;
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (line.raw_uid && line.consumption_kg > 0) {
          await db.execute(sql`
            INSERT INTO need (recipe_id, sku_id, period, gross_qty, adjustment, net_qty, version, created_at)
            VALUES (${recipeId}, ${skuId}, ${period}, ${line.consumption_kg}, 0, ${line.consumption_kg}, ${recipeVersion}, ${new Date().toISOString()})
          `);
        }
      }
...
/** Set recipe status + recalculate need + return stock on cancel */
export async function pgSetRecipeStatus(recipeUid: string, status: string): Promise<boolean> {
  ...
  const statusMap: Record<string, string> = {
    ...
    'архив': 'archived', 'отменён': 'cancelled', 'удалён': 'archived',
    'plan': 'active', 'archive': 'archived', 'cancel': 'cancelled',
  };
  const pgStatus = statusMap[status] || status;
  ...
  await db.execute(sql`BEGIN`).then(async () => {
    try {
      ...
      // При отмене — возвращаем сырьё на склад
      if (pgStatus === 'cancelled') {
        await returnStockForRecipe(recipeId);
      }
...
/** Возврат сырья на склад при отмене рецепта */
async function returnStockForRecipe(recipeId: number): Promise<void> {
  // Получаем consumption_kg из recipe_item
  const items = await db.execute(sql`
    SELECT ri.sku_id, ri.consumption_kg, s.code
    FROM recipe_item ri JOIN sku s ON ri.sku_id = s.id
    WHERE ri.recipe_id = ${recipeId} AND ri.consumption_kg > 0
  ...
  // Получаем последний снимок склада Полоцк (warehouse_id = 1)
  const latestSnapshot = await db.execute(sql`
    SELECT id, payload_json FROM stock_snapshot 
    WHERE warehouse_id = 1 
    ORDER BY snapshot_date DESC LIMIT 1
  `);

  if (latestSnapshot.rows.length === 0) return;

  const snapshot = latestSnapshot.rows[0] as any;
  const payload = Array.isArray(snapshot.payload_json) ? [...snapshot.payload_json] : [];

  // Возвращаем сырьё: увеличиваем остаток по каждому SKU
  for (const item of items.rows) {
    const skuId = (item as any).sku_id;
    const returnQty = (item as any).consumption_kg;
    
    // Ищем SKU в payload
    const existingItem = payload.find((p: any) => Number(p.sku_id) === skuId);
    if (existingItem) {
      existingItem.qty_kg = round3(Number(existingItem.qty_kg) + returnQty);
    } else {
      payload.push({ sku_id: skuId, qty_kg: returnQty });
    }
  }

  // Обновляем снимок склада
  await db.execute(sql`
    UPDATE stock_snapshot SET payload_json = ${JSON.stringify(payload)}::jsonb 
...
export async function pgUpdateRecipeTons(recipeUid: string, newTons: number): Promise<{
  ...
  const newBaseKg = round3(newTons * 1000);
...
export async function pgWriteLipBatchesBulk(rows: {
  ...
  await db.execute(sql`BEGIN`).then(async () => {
    try {
      // Delete ALL previous kd_file rows (not just today's) to prevent duplicates
      await db.execute(sql`DELETE FROM lip_batch WHERE source = 'kd_file'`);
...
export async function pgMergeRawMaterials(sourceUid: string, targetUid: string, rename?: { full_name?: string; short_name?: string }) {
  ...
  try {
    ...
    if (sourceName) await db.execute(sql`INSERT INTO sku_alias (sku_id, alias, source) VALUES (${targetId}, ${sourceName}, 'merge') ON CONFLICT DO NOTHING`);
...
export async function pgPartialArchive(recipeUid: string, producedTons: number) {
  ...
  const producedFactor = producedTons / originalTons;
  const remainingFactor = 1 - producedFactor;

  await db.execute(sql`BEGIN`);
  try {
    // 1. Читаем ИСХОДНЫЕ consumption_kg ДО масштабирования
    const originalItems = await db.execute(sql`SELECT sku_id, dose_kg_per_t, norm_g_per_t, consumption_kg, match_status FROM recipe_item WHERE recipe_id = ${recipeId}`);

    // 2. Обновляем recipe_item для архивной части: consumption = original * producedFactor
    await db.execute(sql`UPDATE recipe_item SET consumption_kg = ROUND((consumption_kg * ${producedFactor})::numeric, 3) WHERE recipe_id = ${recipeId} AND consumption_kg > 0`);

    // 3. Update batch_t and base_batch_kg on current recipe (архивная часть)
    await db.execute(sql`UPDATE recipe SET batch_t = ${producedTons}, base_batch_kg = ROUND(${producedTons * 1000}, 3) WHERE id = ${recipeId}`);

    // 4. Recalculate need for архивная часть
    await db.execute(sql`DELETE FROM need WHERE recipe_id = ${recipeId}`);
    ...
    const period = new Date().toISOString().slice(0, 7);
    for (const item of items.rows) {
      await db.execute(sql`INSERT INTO need (recipe_id, sku_id, period, gross_qty, adjustment, net_qty, version, created_at) VALUES (${recipeId}, ${(item as any).sku_id}, ${period}, ${(item as any).consumption_kg}, 0, ${(item as any).consumption_kg}, 1, ${new Date().toISOString()})`);
    }

    // 5. Archive the current recipe
    await db.execute(sql`UPDATE recipe SET status = 'archived' WHERE id = ${recipeId}`);

    // 6. If there's remainder, create a new recipe
    const remaining = originalTons - producedTons;
    let newRecipeUid: string | null = null;
    if (remaining > 0.001) {
      // Get max version for this code to avoid unique constraint violation
      const maxVerResult = await db.execute(sql`SELECT COALESCE(MAX(version), 0) + 1 as next_version FROM recipe WHERE code = (SELECT code FROM recipe WHERE id = ${recipeId})`);
      const nextVersion = (maxVerResult.rows[0] as any).next_version;
      
      const newRecResult = await db.execute(sql`INSERT INTO recipe (recipe_uid, code, name, status, version, batch_t, base_batch_kg, active_from, is_partial) VALUES (${recipeUid + '_remaining_' + Date.now()}, (SELECT code FROM recipe WHERE id = ${recipeId}), (SELECT name FROM recipe WHERE id = ${recipeId}), 'active', ${nextVersion}, ${remaining}, ROUND(${remaining * 1000}, 3), ${new Date().toISOString().split('T')[0]}, true) RETURNING recipe_uid, id`);
      newRecipeUid = (newRecResult.rows[0] as any).recipe_uid;
      const newRecipeId = (newRecResult.rows[0] as any).id;

      // Copy recipe_items: consumption = ORIGINAL * remainingFactor (не от масштабированного!)
      const newPeriod = new Date().toISOString().slice(0, 7);
      for (const item of originalItems.rows) {
        const originalCons = (item as any).consumption_kg || 0;
        const newCons = round3(originalCons * remainingFactor);
        if (newCons > 0) {
          ...
          await db.execute(sql`INSERT INTO need (recipe_id, sku_id, period, gross_qty, adjustment, net_qty, version, created_at) VALUES (${newRecipeId}, ${(item as any).sku_id}, ${newPeriod}, ${newCons}, 0, ${newCons}, 1, ${new Date().toISOString()})`);
        }
      }
    }
