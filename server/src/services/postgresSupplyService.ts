...
export async function pgPartialArchive(recipeUid: string, producedTons: number) {
  ...
  try {
    ...
    if (remaining > 0.001) {
      ...
      const newRecResult = await db.execute(sql`INSERT INTO recipe (recipe_uid, code, name, status, version, batch_t, base_batch_kg, active_from) VALUES (${recipeUid + '_remaining_' + Date.now()}, (SELECT code FROM recipe WHERE id = ${recipeId}), (SELECT name FROM recipe WHERE id = ${recipeId}), 'active', ${nextVersion}, ${remaining}, ${remaining * 1000}, ${new Date().toISOString().split('T')[0]}) RETURNING recipe_uid, id`);
