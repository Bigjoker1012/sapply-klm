...
/**
 * Достаёт код рецепта (напр. «Д-П60-3/Б20/ПЛЦ-113», «П60-3», «ПКР-2», «КК-61-1»)
 * из произвольной строки — текстового слоя документа ИЛИ имени файла.
 ...
 * Поддерживает форматы: Д-П60-3/Б20/ПЛЦ-113, П60-3, ПКР-2, КК-61-1/Б20/ПЛЦ-178.
 */
export function extractRecipeCode(s: string): string {
  ...
  // Код без Д- префикса: ищем паттерн типа П60-3, ПКР-2, КК-61-1, КД-18 и т.д.
  // Алфавитные + дефис + цифры, возможно с / сегментами
  const shortMatch = s.match(/\b([А-ЯЁA-Z]{2,4}-[А-Яа-яЁёA-Z0-9 _\/-]{2,})/i);
