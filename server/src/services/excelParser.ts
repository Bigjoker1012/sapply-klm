...
export function parseRecipeExcel(buffer: Buffer): {
  ...
  for (let r = 0; r < Math.min(20, rows.length); r++) {
    ...
    // заказчика (ОАО/ООО/«…комбинат хлебопродуктов») и НЕ текст согласования,
    // иначе в списке вместо кода рецепта показывалось «ОАО Полоцкий комбинат»
    // или «Согласовано: Утверждаю:».
    if (!recipeName && line.length > 4 && !/^\d/.test(line) && !isOrgName(line) && !isApprovalText(line)) {
...
/** Похоже ли на текст согласования/утверждения, а не на название рецепта. */
export function isApprovalText(s: string): boolean {
  return /согласован|утверждаю|согласовани|утверждени|допущен|разрешено/i.test(s);
}

/**
 * Определяет тип продукта по коду рецепта и возвращает полное название.
 * Например: П60-3/Б20/ПЛЦ-25 → «Премикс П60-3/Б20/ПЛЦ-25»
 *           ПКР-2/Б20/ПЛЦ-12 → «Премикс-концентрат ПКР-2/Б20/ПЛЦ-12»
 *           КК-61/Б20/ПЛЦ-10 → «Комбикорм-концентрат КК-61/Б20/ПЛЦ-10»
 */
export function recipeFullName(code: string): string {
  ...
  else if (/^КК/.test(prefix)) type = "Комбикорм-концентрат";
...
/**
 * Достаёт код рецепта (напр. «Д-П60-3/Б20/ПЛЦ-113», «П60-3/Б20/ПЛЦ-25»,
 * «ПКР-2/Б20/ПЛЦ-12», «КК-61-1/Б20/ПЛЦ-178») из произвольной строки —
 * текстового слоя документа ИЛИ имени файла.
 * Код может начинаться с «Д-» или без префикса, содержит сегменты через «/»
 * (в файле — через «_» или пробел) и оканчивается на «…Ц-NNN» или просто буквы+цифры.
 */
export function extractRecipeCode(s: string): string {
  if (!s) return "";
  // Полный код с Д- префиксом и ПЛЦ-NNN окончанием (старый формат)
  const fullMatch = s.match(/Д-[А-Яа-яЁёA-Z0-9 _\/-]*?Ц-?\s*\d+/i);
  if (fullMatch) return fullMatch[0].trim();
  // Полный код без Д- префикса: П60-3/Б20/ПЛЦ-25, ПКР-2/Б20/ПЛЦ-12, КК-61/Б20/ПЛЦ-10
  // 1-4 буквы + дефис + цифры/буквы + необязательные сегменты / _ пробел
  const fullShort = s.match(/\b([А-ЯЁA-Z]{1,4}-[А-Яа-яЁёA-Z0-9]+(?:[\/ _-][А-Яа-яЁёA-Z0-9]+)*)/i);
  if (fullShort) return fullShort[1].replace(/[ _]+/g, "/").replace(/\/{2,}/g, "/").trim();
  return "";
}
