 ...
 * "Витамин D(кальция пантетонат) В3" → "в3" (пропускает скобки, берёт последний суффикс).
...
/**
 * Извлекает суффикс из ОРИГИНАЛЬНОГО имени (без нормализации раскладки).
 * "Витамин D(кальция пантетонат) В3" → "в3" (оригинал, не "d3").
 * Нужен чтобы "В3" (рус.) НЕ матчился на "D3" (лат.) — это разные витамины.
 */
...
export async function writePlantStock(rows: { raw_uid: string; name_from_source: string; qty: number; source_file: string }[]): Promise<void> {
  ...
  // Сохраняем историю (старые даты), заменяем сегодняшний снимок
  ...
  // Безопасная замена: СНАЧАЛА пишем данные, ПОТОМ подчищаем хвост.
  // Если clear сорвётся после write — лишние строки останутся, но данные не потеряются.
  await writeRange("PlantStock", `A2:G${allRows.length + 1}`, allRows);
  if (existing.length > allRows.length) {
    await clearRange("PlantStock", `A${allRows.length + 2}:G${existing.length + 1}`);
  }
}
...
export async function deleteRecipe(recipe_uid: string): Promise<number> {
  // Удаляем строки рецепта (col B = recipe_uid)
  const lines = await readRange("RecipeLines", "A2:L5000");
  const keepLines = lines.filter(r => String(r[1]) !== recipe_uid);
  if (keepLines.length !== lines.length) {
    await clearRange("RecipeLines", "A2:L5000");
    if (keepLines.length) await writeRange("RecipeLines", `A2:L${keepLines.length + 1}`, keepLines);
  }
  // Удаляем потребность
