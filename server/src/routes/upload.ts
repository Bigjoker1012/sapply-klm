...
router.post("/polotsk", upload.single("file"), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!parsed.length) {
      return res.status(400).json({ error: "Файл не содержит распознанных строк остатков" });
    }
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if (!Number.isFinite(qty) || qty <= 0) {
        warnings.push(`Строка ${i + 1} ("${row.rawName}"): некорректное количество (${qty}) — пропущена`);
        continue;
      }
      const rawUid = matchMap.get(row.rawName);
      if (rawUid) {
        stockRows.push({ raw_uid: rawUid, name_from_source: row.rawName, qty, source_file: up.originalname });
        matched++;
      } else {
        queueItems.push({ text: row.rawName, source_type: "polotsk", file_name: up.originalname, qty, source_warehouse: "polotsk" });
        unmatched++;
      }
    }

    if (!stockRows.length && !queueItems.length) {
      return res.status(400).json({
        error: "Файл не содержит валидных строк остатков",
        warnings,
      });
    }
    ...
    res.json({ ok: true, total: parsed.length, matched, unmatched, warnings, message: `Загружено: ${matched}, не распознано: ${unmatched}` + (warnings.length ? `, предупреждений: ${warnings.length}` : "") });
  } catch (err: any) {
...
router.post("/lipkovskaya", upload.single("file"), async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      const rawUid = matchMap.get(row.rawName);
      if (rawUid) {
        stockRows.push({ raw_uid: rawUid, name_from_source: row.rawName, qty, source: "excel_file" });
        matched++;
      } else {
        queueItems.push({ text: row.rawName, source_type: "lipkovskaya", file_name: up.originalname, qty, source_warehouse: "lipkovskaya" });
        unmatched++;
      }
    }
    ...
    await Promise.all([
      stockRows.length ? writeLipStockBatch(stockRows) : Promise.resolve(),
      queueItems.length ? addToReviewQueueBatch(queueItems) : Promise.resolve(),
    ]);
    ...
    res.json({
      ...
      unmatched,
      warnings,
      message: `Загружено: ${matched}, не распознано: ${unmatched}` + (warnings.length ? `, предупреждений: ${warnings.length}` : ""),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post("/recipe", upload.single("file"), async (req: Request, res: Response) => {
  ...
  try {
    ...
    const admission = await withStockMutation(async () => {
      // Код рецепта — главный идентификатор в списке. Если из содержимого не
