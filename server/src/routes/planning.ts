 ...
 *
 * «Кол-во сегодня» = остатки Полоцк + Липковская + в пути (без вычета рецептов).
 * Среднемесячный расход авто НЕ считаем (нет истории расхода) — только ручной ввод;
 * иначе поле пустое. coefficient — «ручной коэф-т» (запас под срок поставки):
 * 1.0–2.0, по умолч. 1. Статус считается на СЕРВЕРЕ единым модулем
 * services/planningStatus.ts (тот же, что светофор «Главной»): коэф-т
 * потребности = наличие/ср.расход, итог = коэф-т потребности / ручной коэф-т;
 * >1.5 норма, 1–1.5 контроль, 0.6–1 закупка, <0.6 срочная закупка.
 * Без ручного расхода статус не считается.
 */
