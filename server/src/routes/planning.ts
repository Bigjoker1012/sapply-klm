 ...
 *   GET   /api/planning            — список SKU + кол-во сегодня + дефицит + настройки
 *   PATCH /api/planning/:raw_uid   — сохранить коэффициент / ручной ввод / расход
 *
 * «Кол-во сегодня» = остатки Полоцк + Липковская + в пути (реальный остаток).
 * «Дефицит» = остаток за вычетом потребности по рецептам (может быть отрицательным).
 * Среднемесячный расход: ручной имеет приоритет, иначе авто-расчёт из выработанных (архивных) рецептов за 3 месяца;
 * иначе поле пустое. coefficient — «ручной коэф-т» (запас под срок поставки): 1.0–2.0,
 * по умолч. 1. Статус считается на СЕРВЕРЕ единым модулем services/planningStatus.ts
 * (тот же, что светофор «Главной»): коэф-т потребности = дефицит/ср.расход,
 * итог = коэф-т потребности / ручной коэф-т; >1.5 норма, 1–1.5 контроль, 0.6–1 закупка,
...
router.get("/", async (_req: Request, res: Response) => {
  try {
    // Единый расчёт планирования (тот же, что светофор на «Главной»): статус,
    // коэф-т потребности и итог считаются на сервере — см. planningStatus.ts.
