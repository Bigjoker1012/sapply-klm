...
export default function App() {
  // --- СОСТОЯНИЕ ДЛЯ ОСТАТКОВ И МОНИТОРА ДЕФИЦИТА ---
  const [searchQuery, setSearchQuery] = useState('');
  const [activeFilter, setActiveFilter] = useState<'ALL' | 'CRITICAL' | 'TRANSFER'>('ALL');

  // Начальные данные таблицы (как на макете!)
  ...
  const handleAddTransit = (e: React.FormEvent) => {
    ...
    // Обновляем количество "В пути" в основной таблице
    setInventory(prev => prev.map(item => {
      if (item.name === selectedMaterial) {
        ...
        return {
          ...
          free: item.polotsk + item.lipki + newTransitSum // условный расчет свободного остатка
        };
      }
      return item;
    }));
  ...
  // Вспомогательная функция красивой даты
  ...
  // Фильтрация и поиск для таблицы
    ...
    <div className="min-h-screen bg-[#0b0f19] text-slate-100 p-4 sm:p-6 font-sans antialiased">
      ...
      <section className="mb-6 bg-[#111a2e] border border-slate-850 rounded-xl p-5 shadow-lg">
        ...
        <form onSubmit={handleAddTransit} className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end mb-4">
          ...
          <button 
            type="submit"
            className="w-full bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 text-slate-200 py-2 rounded-lg text-sm font-semibold transition"
          >
            + Добавить поставку
          </button>
        </form>
            ...
            <tbody className="divide-y divide-slate-800/60 text-xs font-medium">
              {filteredInventory.length === 0 ? (
                ...
                filteredInventory.map((item, index) => {
                  ...
                  // Расчет классов строки в зависимости от статуса
                  ...
                  return (
                    <tr key={item.id} className={`transition-colors ${rowBg}`}>
                      ...
                      <td className={`py-3 px-4 text-right font-mono ${freeColorClass}`}>
