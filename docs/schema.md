...
1. **Версии рецептов** — храним полную копию `recipe_item` на каждую версию, или дельты? (предлагаю полные копии, проще и БД маленькая).
2. **Цены и валюты** — нужен ли отдельный `fx_rate` (курсы валют) для оценки запасов в одной валюте?
3. **Многозаводская модель в будущем** — если появятся ещё площадки кроме Полоцка/Липковской, схема не меняется, добавляются строки в `warehouse`.
4. **Партии vs «оборотка»** — есть смысл сразу заводить полноценные партии (мой выбор) или сначала держать только агрегированный остаток `(sku, warehouse, qty_kg)` и партии добавить позже? Партии нужны минимум для срока годности — поэтому сразу.
5. **Дозировка** — `dose_kg_per_t` (на тонну премикса) или `dose_pct` (% по массе)? Сейчас в листах встречается и то и другое — определимся.
